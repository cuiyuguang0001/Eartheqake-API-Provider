...
public class WorkDomain {
     ...
     * @return
     */
    public Map<String ,Object> EqFormModelZQJBAdd(HttpServletRequest httpServletRequest){
        ...
        eq.setXzq(httpServletRequest.getParameter("xzq"));
        eq.setXxmc(httpServletRequest.getParameter("xxmc"));
        eq.setJd(httpServletRequest.getParameter("jd"));
        eq.setWd(httpServletRequest.getParameter("wd"));
        eq.setZhqk(httpServletRequest.getParameter("zhqk"));
        eq.setZqlx(httpServletRequest.getParameter("zqlx"));
        String fids = httpServletRequest.getParameter("fids");
        eq.setFj((fids == null || fids.equals("")) ? "0" : "1");

        //添加zqjb表信息+
        if(zqjbMapper.zqjbAdd(eq))
        {
            //添加eq_form_mod_record表信息
            if(!eqFormModRecordAdd(httpServletRequest, eq.getId() + "", session.getUid()))
                return null;
            //添加filefollow表信息
            fileDomain.fileFollowAdd(fids,eq.getId() + "", "zqjb");
     ...
     * @return
     */
    public Map<String, Object> EqFormModelFWDCAdd(HttpServletRequest httpServletRequest)
    {
     ...
     * @return
     */
    public Map<String ,Object> EqFormModelDTDCAdd(HttpServletRequest httpServletRequest)
    {
        //检查登陆状态
        Session session = userDomain.getSessionBySid(httpServletRequest.getParameter("sid"));
        if(session == null || session.getUid().equals("0"))
     ...
     * @return
     */
    public boolean eqFormModRecordAdd(HttpServletRequest httpServletRequest, String id, String uid)
    {
        EQ_form_record form = new EQ_form_record();
        form.setAddress(httpServletRequest.getParameter("xzq"));
        form.setTitle(httpServletRequest.getParameter("xxmc"));
        form.setDateline(CommonUtil.getTineLine());
        form.setUid(uid);
        form.setEqid(httpServletRequest.getParameter("eqid"));
